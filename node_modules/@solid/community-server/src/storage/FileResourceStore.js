...
exports.FileResourceStore = void 0;
...
const { extname, join: joinPath, normalize: normalizePath } = path_1.posix;
/**
...
class FileResourceStore {
     ...
     */
    constructor(runtimeConfig, interactionController, metadataController) {
        this.runtimeConfig = runtimeConfig;
        this.interactionController = interactionController;
        this.metadataController = metadataController;
    }
    get baseRequestURI() {
        return Util_1.trimTrailingSlashes(this.runtimeConfig.base);
    }
    get rootFilepath() {
        return Util_1.trimTrailingSlashes(this.runtimeConfig.rootFilepath);
    }
    /**
    ...
    async addResource(container, representation) {
        ...
        if (!representation.binary) {
            throw new UnsupportedMediaTypeHttpError_1.UnsupportedMediaTypeHttpError('FileResourceStore only supports binary representations.');
        }
        ...
        const linkTypes = (_a = representation.metadata.linkRel) === null || _a === void 0 ? void 0 : _a.type;
        let metadata;
        if (raw.length > 0) {
    ...
    async deleteResource(identifier) {
        ...
        if (path === '' || Util_1.ensureTrailingSlash(path) === '/') {
            throw new MethodNotAllowedHttpError_1.MethodNotAllowedHttpError('Cannot delete root container.');
        }
        ...
        let stats;
        try {
            stats = await fs_1.promises.lstat(path);
        }
        catch (error) {
        ...
        if (stats.isFile()) {
            ...
            await this.deleteDirectory(Util_1.ensureTrailingSlash(path));
        }
        else {
            throw new NotFoundHttpError_1.NotFoundHttpError();
        }
    ...
    async getRepresentation(identifier) {
        ...
        if (stats.isDirectory()) {
            return await this.getDirectoryRepresentation(Util_1.ensureTrailingSlash(path), stats);
        }
        throw new NotFoundHttpError_1.NotFoundHttpError();
    }
    /**
    ...
    async setRepresentation(identifier, representation) {
        ...
        const [, path, slug] = (_a = /^(.*\/)([^/]+\/?)?$/u.exec(this.parseIdentifier(identifier))) !== null && _a !== void 0 ? _a : [];
        if ((typeof path !== 'string' || normalizePath(path) === '/') && typeof slug !== 'string') {
            throw new ConflictHttpError_1.ConflictHttpError('Container with that identifier already exists (root).');
        }
        const { raw } = representation.metadata;
        const linkTypes = (_b = representation.metadata.linkRel) === null || _b === void 0 ? void 0 : _b.type;
        let metadata;
        if (raw.length > 0) {
            metadata = streamify_array_1.default(raw);
        }
        // Create a new container or resource in the parent container with a specific name based on the incoming headers.
        const isContainer = this.interactionController.isContainer(slug, linkTypes);
        const newIdentifier = this.interactionController.generateIdentifier(isContainer, slug);
    ...
    async deleteFile(path) {
        ...
        try {
            await fs_1.promises.unlink(`${path}.metadata`);
        }
        catch (_) {
    ...
    async deleteDirectory(path) {
        ...
        const match = files.find((file) => !file.startsWith('.metadata'));
        if (typeof match === 'string') {
            throw new ConflictHttpError_1.ConflictHttpError('Container is not empty.');
        }
        // Only delete the metadata file as auxiliary resource because this is the only file created by this store.
        try {
            await fs_1.promises.unlink(joinPath(path, '.metadata'));
        }
        catch (_) {
            // It's ok if there was no metadata file.
        }
     ...
     * @returns The corresponding Representation.
     */
    async getFileRepresentation(path, stats) {
        const readStream = fs_1.createReadStream(path);
        const contentType = mime_types_1.contentType(extname(path));
        let rawMetadata = [];
        try {
            const readMetadataStream = fs_1.createReadStream(`${path}.metadata`);
     ...
     * @returns The corresponding Representation.
     */
    async getDirectoryRepresentation(path, stats) {
        const files = await fs_1.promises.readdir(path);
        const quads = [];
        ...
        let rawMetadata = [];
        try {
            const readMetadataStream = fs_1.createReadStream(joinPath(path, '.metadata'));
            rawMetadata = await arrayify_stream_1.default(readMetadataStream);
        }
        catch (_) {
            // Metadata file doesn't exist so lets keep `rawMetaData` an empty array.
        }
        return {
            binary: false,
            data: streamify_array_1.default(quads),
            metadata: {
                raw: rawMetadata,
                dateTime: stats.mtime,
     ...
     */
    async getDirChildrenQuadRepresentation(files, path, containerURI) {
        const quads = [];
        for (const childName of files) {
            try {
                const childURI = this.mapFilepathToUrl(joinPath(path, childName));
                const childStats = await fs_1.promises.lstat(joinPath(path, childName));
     ...
     */
    async setFileRepresentation(path, newIdentifier, data, metadata) {
        // (Re)write file for the resource if no container with that identifier exists.
        let stats;
        try {
            stats = await fs_1.promises.lstat(joinPath(this.rootFilepath, path, newIdentifier));
        }
        catch (error) {
        ...
        throw new ConflictHttpError_1.ConflictHttpError('Container with that identifier already exists.');
    }
    /**
     ...
     * @param metadata - Optional metadata.
     */
    async setDirectoryRepresentation(path, newIdentifier, metadata) {
        // Create a container if the identifier doesn't exist yet.
        try {
            await fs_1.promises.access(joinPath(this.rootFilepath, path, newIdentifier));
            throw new ConflictHttpError_1.ConflictHttpError('Resource with that identifier already exists.');
        }
        catch (error) {
            if (error instanceof ConflictHttpError_1.ConflictHttpError) {
                throw error;
            }
     ...
     */
    async createFile(path, resourceName, data, allowRecursiveCreation, metadata) {
        ...
        let stats;
        try {
            stats = await fs_1.promises.lstat(joinPath(this.rootFilepath, path));
        }
        catch (error) {
        ...
        if (!stats.isDirectory()) {
            throw new MethodNotAllowedHttpError_1.MethodNotAllowedHttpError('The given path is not a valid container.');
        }
        else {
            ...
            try {
                ...
                await fs_1.promises.unlink(joinPath(this.rootFilepath, path, `${resourceName}.metadata`));
                throw error;
            }
        }
    }
     ...
     */
    async createContainer(path, containerName, allowRecursiveCreation, metadata) {
        const fullPath = Util_1.ensureTrailingSlash(joinPath(this.rootFilepath, path, containerName));
        // If recursive creation is not allowed, check if the parent container exists and then create the child directory.
        try {
            if (!allowRecursiveCreation) {
                const stats = await fs_1.promises.lstat(joinPath(this.rootFilepath, path));
                if (!stats.isDirectory()) {
                    throw new MethodNotAllowedHttpError_1.MethodNotAllowedHttpError('The given path is not a valid container.');
                }
            }
            await fs_1.promises.mkdir(fullPath, { recursive: allowRecursiveCreation });
        }
        catch (error) {
            if (error instanceof MethodNotAllowedHttpError_1.MethodNotAllowedHttpError) {
                throw error;
            }
            throw new MethodNotAllowedHttpError_1.MethodNotAllowedHttpError();
        }
     ...
     * @param data - The data to be put in the file.
     */
    async createDataFile(path, data) {
...
exports.FileResourceStore = FileResourceStore;
//# sourceMappingURL=FileResourceStore.js.map